...
import updateAccountWithComponent from "@salesforce/apex/UpdateAccountController.updateAccountWithComponent";

export default class UpdateAccountWithApexComponent extends LightningElement {
  @api recordId;
    ...
    if ((this.CPF || this.CNPJ) && this.accountNumber && this.accountName) {
      updateAccountWithComponent({
        ...
        .catch((error) => {
          ...
          const errorMessage = error.body?.message.includes(
            "Número do cliente é inválido"
          )
            ? "Número do cliente é inválido"
            : "Não foi possível concluir a atualização. Por favor, tente novamente!";
          alert(errorMessage);
        });
    } else {
